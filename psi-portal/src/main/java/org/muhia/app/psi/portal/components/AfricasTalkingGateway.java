...
public class AfricasTalkingGateway {
    ...
    public JSONArray fetchMessages(int lastReceivedId_) throws Exception {
        String requestUrl = getSmsUrl() + "?" + URLEncoder.encode("username", "UTF-8") + "=" + URLEncoder.encode(_username, "UTF-8") + "&" + URLEncoder.encode("lastReceivedId", "UTF-8") + "=" + URLEncoder.encode(Integer.toString(lastReceivedId_), "UTF-8");
    ...
    public JSONObject createSubscription(String phoneNumber_, String shortCode_, String keyword_) throws Exception {
        ...
        if (responseCode != HTTP_CODE_CREATED) throw new Exception(response.toString());
    ...
    public JSONArray call(String from_, String to_) throws Exception {
        ...
        if (jsObject.getString("errorMessage").equals("None")) return jsObject.getJSONArray("entries");
    ...
    public void uploadMediaFile(String url_) throws Exception {
        ...
        if (!jsObject.getString("errorMessage").equals("None")) throw new Exception(jsObject.getString("errorMessage"));
    ...
    public JSONObject initiateMobilePaymentCheckout(String productName_, String phoneNumber_, String currencyCode_, Double amount_, Map<String, String> metadata_) throws Exception {
        JSONObject requestBody = new JSONObject().put("username", _username).put("productName", productName_).put("phoneNumber", phoneNumber_).put("currencyCode", currencyCode_).put("amount", amount_).put("metadata", metadata_);
        String response = sendJsonPOSTRequest(requestBody.toString(), getMobilePaymentCheckoutUrl());
        return new JSONObject(response);
    }

    public JSONArray mobilePaymentB2CRequest(String productName_, List<MobilePaymentB2CRecipient> recipients_) throws Exception {
        ...
        JSONObject requestBody = new JSONObject().put("username", _username).put("productName", productName_).put("recipients", jsonRecipients);
    ...
    public JSONObject mobilePaymentB2BRequest(String productName_, HashMap<String, String> providerData_, String currencyCode_, float amount_, HashMap<String, String> metadata_) throws Exception {
        if (!providerData_.containsKey("provider")) throw new Exception("Missing field provider");

        if (!providerData_.containsKey("destinationChannel")) throw new Exception("Missing field destinationChannel");

        if (!providerData_.containsKey("transferType")) throw new Exception("Missing field transferType");
...
class MobilePaymentB2CRecipient {
    ...
    public JSONObject toJSON() throws Exception {
        return new JSONObject().put("phoneNumber", _phoneNumber).put("currencyCode", _currencyCode).put("amount", _amount).put("metadata", _metadata);
    }
}